...
// unique to identity -- lock, tactic, strategy, and interest all have the
...
func TestIdentityHelpNotSpecialCasedAtSubcommandLevel(t *testing.T) {
	for _, arg := range []string{"-h", "--help", "help"} {
		...
		if code != 2 {
			t.Fatalf("cmdIdentity([%s]): exit %d, want 2 (treated as an unknown subcommand, matching lock/tactic/strategy/interest)", arg, code)
		}
